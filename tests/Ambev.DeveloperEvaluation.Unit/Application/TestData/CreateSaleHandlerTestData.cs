...
namespace Ambev.DeveloperEvaluation.Unit.TestData
{
    ...
    {
        public static CreateSaleCommand GenerateValidCommand(Guid saleId)
        {
            var faker = new Faker<CreateSaleCommand>()
                ...
                .RuleFor(s => s.Itens, (f, saleCmd) =>
                {
                    // Usa o SaleId recebido para todos os itens
                    return new Faker<SaleItem>()
                        ...
                        .RuleFor(si => si.SaleId, f => saleId) // Todos os itens terão o mesmo SaleId
